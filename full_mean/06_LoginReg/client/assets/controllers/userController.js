	...
	$scope.login = function() {
		...
		usersFactory.login($scope.loginUser, function(status, response) {
			if (status == false) {
				...
				$cookies.put('user_id', response._id); // set user_id into cookie
				$location.url('/home'); // send user to home partial
			}
		});
	} // end $scope.login
		...
		if($scope.newUser.password != $scope.newUser.confirmPassword) {
			...
			usersFactory.register($scope.newUser, function(status, response) {
				if (status == false) { // if false, there are errors on the registration form
					...
					$location.url('/home'); // send new user to home partial to view user information
				}
			});
		}
	} // end $scope.register
