	...
	$scope.login = function() {
		...
		usersFactory.login($scope.loginUser, function(status, response) {
			if (status == false) {
				...
				$cookies.put('username', response.username); // set username into cookie
				$location.url('/dashboard'); // send user to dashboard partial
			}
		});
	} // end $scope.login
		...
		if($scope.newUser.password != $scope.newUser.confirmPassword) {
			...
			usersFactory.register($scope.newUser, function(status, response) {
				if (status == false) { // if false, there are errors on the registration form
					...
					$cookies.put('username', response.username)  // set user_id in cookie to use in logout process later on
					$location.url('/dashboard'); // send new user to home partial to view user information
				}
			});
		}
	} // end $scope.register
